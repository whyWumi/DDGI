...
def construct_dataloaders(dataset, defs, data_path='~/data', shuffle=True, normalize=True):
    ...
    if dataset == 'CIFAR10':
        ...
        loss_fn = Classification()
    elif dataset == 'myimagenet':
        trainset, validset = _build_myimagenet('E:/data/myimage/', defs.augmentations, normalize)
        loss_fn = Classification()
...
def  _build_myimagenet(data_path, augmentations=True, normalize=True):
    """Define myImageNet with everything considered."""
    data_mean, data_std = myimagenet_mean, myimagenet_std
    ...
    train_size = int(0.8 * len(dataset))  # 训练集占0.8
    val_size = len(dataset) - train_size   # 验证集占0.2
    trainset, validset = random_split(dataset, [train_size, val_size], generator=torch.Generator().manual_seed(0))

    #trainset = torchvision.datasets.ImageNet(root=data_path, split='train', transform=transforms.ToTensor())
    #validset = torchvision.datasets.ImageNet(root=data_path, split='val', transform=transforms.ToTensor())

    if myimagenet_mean is None:
        data_mean, data_std = _get_meanstd(trainset)
    else:
        data_mean, data_std = myimagenet_mean, myimagenet_std
